document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchNavigation() {
        try {
            ...
            if (data.navigation) {
                availableMonths = data.navigation; // Store for later use
    ...
    async function renderAllTables() {
        ...
        for (const monthInfo of availableMonths) {
            try {
                ...
                if (!response.ok) continue; // Skip failed months
                const data = await response.json();
                ...
                console.error(`Failed to load table for ${monthInfo.label}`, error);
                const errorDiv = document.createElement('div');
                errorDiv.innerHTML = `<h2>${monthInfo.label}</h2><p class="error-message">ไม่สามารถโหลดข้อมูลได้</p>`;
                allEventsContainer.appendChild(errorDiv);
            }
        }
    ...
    async function renderAllCalendars() {
        ...
        for (const monthInfo of availableMonths) {
            ...
            grid.id = `calendar-grid-${monthInfo.year}-${monthInfo.month}`; // Unique ID for each grid
            ...
            try {
                ...
                console.error(`Failed to load calendar for ${monthInfo.label}`, error);
                grid.innerHTML = '<p class="error-message">ไม่สามารถโหลดข้อมูลได้</p>';
            }
        }
        hideLoading();
    }
    ...
    function renderNavigation(navData) {
        navigationContainer.innerHTML = ''; // Clear previous
    ...
    function renderSingleCalendar(year, month, data, targetGrid) {
        ...
        for (let i = 0; i < startOffset; i++) {
            targetGrid.appendChild(document.createElement('div')).classList.add('calendar-day', 'empty');
        }
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (data.holidays && data.holidays[dayKey]) {
                ...
                holidayDiv.classList.add('event-item', 'holiday-item');
                holidayDiv.textContent = data.holidays[dayKey];
            ...
            if (eventsForCalendar[dayKey]) {
                ...
                eventsForCalendar[dayKey].forEach(event => {
                    ...
                    if (siteSettings.truncate_event_names === '1' && eventText.length > 25) {
                       eventText = eventText.substring(0, 22) + '...';
                    }
                    ...
                    eventDiv.addEventListener('click', () => {
                        window.location.href = `export_ical.php?id=${event.id}`;
                    });
    ...
    function renderTable(year, month, data) {
        ...
        if (eventList.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="4">ไม่มีกิจกรรมในเดือนนี้</td></tr>';
            return;
        }
    ...
    function renderSearchResults(results) {
        ...
        if (!results || results.length === 0) {
            searchResultsContainer.innerHTML = `<h3>ผลการค้นหา (0 รายการ)</h3><p>ไม่พบกิจกรรมที่ตรงกัน</p>`;
            return;
        }
        ...
        results.forEach(event => {
            html += `<li>
                <h4>${formatResponsibleUnit(event.event_name, event.responsible_unit)}</h4>
                <p><strong>วันที่:</strong> ${event.dates ? event.dates : 'N/A'}</p>
                ${event.notes ? `<p class="notes"><strong>หมายเหตุ:</strong> ${event.notes}</p>` : ''}
            </li>`;
        });
        html += '</ul>';
    ...
    navigationContainer.addEventListener('click', (e) => {
        ...
        if (!target.classList.contains('nav-button')) return;
    ...
    calendarViewBtn.addEventListener('click', () => {
        ...
        if (isAllViewActive) renderAllViews();
    });
    ...
    document.getElementById('submit-report-btn').addEventListener('click', async () => {
        ...
        if (!message.trim()) {
            statusMsg.textContent = 'กรุณากรอกข้อความ';
        ...
        try {
            ...
            statusMsg.textContent = 'เกิดข้อผิดพลาดในการส่ง';
            statusMsg.style.color = 'red';
        }
    });
    ...
    async function initialize() {
        ...
        if (publishStartDate && todayForCompare < publishStartDate) {
            initialYear = publishStartDate.getFullYear();
            initialMonth = publishStartDate.getMonth();
        } else if (publishEndDate && todayForCompare > publishEndDate) {
            initialYear = publishEndDate.getFullYear();
            initialMonth = publishEndDate.getMonth();
        }
        ...
        if (initialButton) {
            initialButton.click(); // Trigger click to handle all logic
        } else {
