...
function format_table_date($date_str, $settings) {
    ...
    try { $date = new DateTime($date_str); } catch (Exception $e) { return $date_str; }
    $day_name_part = '';
    ...
    $month_part = ($month_format === 'full') ? $thai_month_full[(int)$date->format('n')] : $thai_month_short[(int)$date->format('n')];
    $year_part = '';
    $year_ce = (int)$date->format('Y'); $year_be = $year_ce + 543;
    switch ($year_format) {
...
try {
    ...
    if (!isset($pdo)) { throw new Exception("ไม่สามารถเชื่อมต่อฐานข้อมูลได้"); }
    ...
    foreach ($all_event_occurrences as $row) {
        $events_by_day[$row['activity_date']][] = ['id' => $row['id'], 'event_name' => $row['event_name'], 'responsible_unit' => $row['responsible_unit'], 'notes' => $row['notes']];
    }
    ...
    if (!empty($event_ids)) {
        ...
        foreach ($full_events_data as $event) {
            ...
            if ($is_continuous) {
                $start_date = $date_objects[0]; $end_date = end($date_objects);
                $event_list_for_table[] = ['id' => $event['id'], 'sort_key' => ($start_date < $current_month_start) ? 0 : 1, 'sort_date' => $start_date->format('Y-m-d'), 'event_name' => $event['event_name'], 'responsible_unit' => $event['responsible_unit'], 'formatted_dates' => format_table_date($start_date->format('Y-m-d'), $settings) . " - " . format_table_date($end_date->format('Y-m-d'), $settings)];
            } else {
                foreach ($date_objects as $date) {
                    if ($date->format('Y-m') == $current_month_start->format('Y-m')) {
                        $event_list_for_table[] = ['id' => $event['id'], 'sort_key' => 1, 'sort_date' => $date->format('Y-m-d'), 'event_name' => $event['event_name'], 'responsible_unit' => $event['responsible_unit'], 'formatted_dates' => format_table_date($date->format('Y-m-d'), $settings)];
                    }
                }
            }
        }
    ...
    $stmt_holidays = $pdo->prepare("SELECT holiday_date, holiday_name FROM special_holidays WHERE is_hidden = 0 AND YEAR(holiday_date) = ? AND MONTH(holiday_date) = ?");
    $stmt_holidays->execute([$year, $month_num]);
    $holidays_by_day = $stmt_holidays->fetchAll(PDO::FETCH_KEY_PAIR);

    echo json_encode(['events_for_calendar' => $events_by_day, 'event_list_for_table' => $event_list_for_table, 'holidays' => $holidays_by_day]);
} catch (PDOException $e) {
    http_response_code(500); echo json_encode(['error' => 'Database Error: ' . $e->getMessage()]);
} catch (Exception $e) {
    http_response_code(500); echo json_encode(['error' => 'General Error: ' . $e->getMessage()]);
}
